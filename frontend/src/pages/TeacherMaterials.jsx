...
const TeacherMaterials = () => {
    ...
    // Get pre-selected subject and section from navigation state
    ...
    const preSelectedCategory = location.state?.category || 'notes';
    
    const [materials, setMaterials] = useState([]);
    const [loading, setLoading] = useState(true);
    const [activeTab, setActiveTab] = useState(preSelectedCategory);
    ...
    const fetchData = async () => {
        try {
            ...
            // Fetch materials for the pre-selected subject (or all materials if no subject selected)
            if (preSelectedSubject) {
                const params = new URLSearchParams();
                params.append('subject', preSelectedSubject);
                if (preSelectedYear) {
                    params.append('year', preSelectedYear);
                }
                const materialsRes = await API.get(`/materials/subject?${params.toString()}`);
                setMaterials(materialsRes.data || []);
            } else {
                // Fetch all materials if no subject is pre-selected
    ...
    // Get unique subjects from teaching subjects
    ...
    const getYearsForSubject = () => {
        if (!uploadData.subject) return [];
        const subject = teachingSubjects.find(ts => ts.subject === uploadData.subject);
    ...
    const handleUpload = async (e) => {
        ...
        try {
            ...
            fetchMaterials(uploadData.subject);
        } catch (err) {
    ...
    const fetchMaterials = async (subject) => {
        try {
            ...
            const response = await API.get(`/materials/subject?${params.toString()}`);
    ...
    const handleSubjectChange = (subject) => {
        setUploadData(prev => ({ ...prev, subject, year: '' }));
    ...
    return (
        <motion.div 
            className="teacher-materials-page"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
        >
            ...
            >
                <div className="header-content">
                    ...
                    <div>
                        <h1>My Materials - {preSelectedSubject} {preSelectedYear ? `Year ${preSelectedYear}` : ''}</h1>
                        <p>Upload and manage study materials for your subject</p>
                    </div>
                </div>
                <button 
            ...
            {getUniqueSubjects().length > 0 && (
                <div className="subject-selector">
                    ...
                    <select 
                        value={preSelectedSubject || uploadData.subject}
                        ...
                        >
                            ...
                            <div className="material-info">
                                ...
                                <div className="material-meta">
                                    ...
                                    <span className="year-badge">Year {material.year}</span>
                        ...
                        <form onSubmit={handleUpload} className="upload-form">
                            ...
                            <div className="form-group">
                                ...
                                <select 
                                    ...
                                    onChange={(e) => setUploadData({...uploadData, subject: e.target.value, year: ''})}
                                    required
                                >
                                    <option value="">Select Subject</option>
                                    {getUniqueSubjects().map(subject => (
