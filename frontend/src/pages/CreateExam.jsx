...
import { 
    Clock, ListChecks, Plus, Check, FileText, Trash2, 
    X, Save, Upload, Image as ImageIcon, AlignLeft, List, Lock,
    BookOpen, Calendar
} from 'lucide-react';
...
const CreateExam = () => {
    ...
    // Check if coming from SubjectAdmin with pre-filled data
    const prefillData = location.state || {};
    const isPrefilled = prefillData.subject && prefillData.year;
    ...
    // Questions added directly to exam (inline)
    const [examQuestions, setExamQuestions] = useState([
        {
            id: Date.now(),
            questionText: '',
            questionType: 'mcq', // 'mcq' or 'detailed'
            options: ['', '', '', ''],
            correctAnswer: '',
            correctAnswers: [],
            category: '',
    ...
    useEffect(() => {
        ...
        // Set default section from user's section if they're a teacher
        if (user.section) {
            setExamData(prev => ({ ...prev, section: user.section }));
        }
        
        // Fetch teacher's assigned subjects
        fetchTeachingSubjects();
        
        // If pre-filled from SubjectAdmin, set the values
        if (prefillData.subject) {
            setExamData(prev => ({
                ...prev,
                subject: prefillData.subject,
                year: prefillData.year || '',
                section: prefillData.section || user.section || ''
            }));
        }
        
        setLoading(false);
    }, [user, navigate, prefillData]);
    ...
    // Check if option is selected as correct (for multiple correct)
    ...
    const validateQuestions = () => {
        ...
        examQuestions.forEach((q, index) => {
            ...
            if (q.questionType === 'mcq') {
                ...
                // Detailed questions don't need correct answer selection
            }
        });
        return errors;
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            for (const q of examQuestions) {
                ...
                formData.append('category', q.category || examData.subject || 'Exam');
            ...
            const response = await API.post('/teacher/create-exam', examPayload);
            ...
            >
                <div className="header-content">
                    ...
                    <div>
                        ...
                        <p>Add exam details and questions all at once</p>
                    </div>
                </div>
            </motion.div>
                            ...
                            <div className="form-row">
                                <div className="form-group">
                                    ...
                                    <div className="fixed-value">
                                        <BookOpen size={16} />
                                        <span>{examData.subject || 'Not set'}</span>
                                    </div>
                                </div>
                                ...
                                <div className="form-group">
                                    ...
                                    <div className="fixed-value">
                                        <Calendar size={16} />
                                        <span>{examData.year ? `Year ${examData.year}` : 'Not set'}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="schedule-section">
                                <div className="schedule-toggle">
                                    ...
                                    <span className="schedule-label">
                                        <Clock size={16} />
                                        Schedule Exam (Set availability time)
                                    </span>
                                </div>
                                ...
                                {examData.isScheduled && (
                                    ...
                                    >
                                        ...
                                        <p className="schedule-hint">
                                            Students can only attempt the exam during this time window.
                                        </p>
                                    </motion.div>
                                )}
                            </div>
...
<button
                                                ...
                                                onClick={() => {
                                                    updateQuestion(q.id, 'questionType', 'mcq');
                                                    // Reset multiple correct when switching to MCQ
                                                    if (q.isMultipleCorrect) {
                                                        updateQuestion(q.id, 'isMultipleCorrect', false);
                                                        updateQuestion(q.id, 'correctAnswers', []);
                                                    }
                                                    // Clear correctAnswer when switching from descriptive to MCQ to ensure clean state
                                                    if (q.questionType === 'detailed') {
                                            ...
                                            <button
                                                ...
                                                onClick={() => {
                                                    updateQuestion(q.id, 'questionType', 'detailed');
                                                    // Reset multiple correct when switching to descriptive
                                                    if (q.isMultipleCorrect) {
                                                        updateQuestion(q.id, 'isMultipleCorrect', false);
                                                        updateQuestion(q.id, 'correctAnswers', []);
                                                    }
                                                        ...
                                                        <input
                                                            ...
                                                            onChange={(e) => {
                                                                if (e.target.checked) {
                                                                    ...
                                                                    updateQuestion(q.id, 'isMultipleCorrect', true);
                                                                    updateQuestion(q.id, 'correctAnswers', []);
                                                                    updateQuestion(q.id, 'correctAnswer', '');
                                                                } else {
...
.toggle-switch {
                ...
                .detailed-answer-section .form-label {
                    font-size: 0.875rem;
                    color: #6b7280;
                }
